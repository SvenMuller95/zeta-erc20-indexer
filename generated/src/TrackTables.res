...
let trackAllTables = async () => {
  ...
  let _ = await trackTable(~tableName="ZETA_Approval")
  let _ = await createSelectPermissions(~tableName="ZETA_Approval")
  let _ = await trackTable(~tableName="ZETA_ClaimReward")
  let _ = await createSelectPermissions(~tableName="ZETA_ClaimReward")
  let _ = await trackTable(~tableName="ZETA_Deposit")
  let _ = await createSelectPermissions(~tableName="ZETA_Deposit")
  let _ = await trackTable(~tableName="ZETA_NewRewardPool")
  let _ = await createSelectPermissions(~tableName="ZETA_NewRewardPool")
  let _ = await trackTable(~tableName="ZETA_Transfer")
  let _ = await createSelectPermissions(~tableName="ZETA_Transfer")
  let _ = await trackTable(~tableName="ZETA_Withdrawal")
  let _ = await createSelectPermissions(~tableName="ZETA_Withdrawal")
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_Approval",
    ~mappedEntity="ZETA_Approval",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_ClaimReward",
    ~mappedEntity="ZETA_ClaimReward",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_Deposit",
    ~mappedEntity="ZETA_Deposit",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_NewRewardPool",
    ~mappedEntity="ZETA_NewRewardPool",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_Transfer",
    ~mappedEntity="ZETA_Transfer",
  )
  let _ = await createEntityRelationship(
    ~tableName="EventsSummary",
    ~relationshipType="array",
    ~derivedFromFieldKey="eventsSummary",
    ~relationalKey="zETA_Withdrawal",
    ~mappedEntity="ZETA_Withdrawal",
  )
}
